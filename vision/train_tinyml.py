...
# 默认输出到 MCU 工程目录
DEFAULT_OUTPUT = os.path.join("D:", os.sep, "e2studio_test", "Robotic_arm", "src", "tinyml_weights.h")
...
def export_c_header(w1, b1, w2, b2, w3, b3, filepath):
    with open(filepath, "w", encoding="utf-8") as f:
        ...
        f.write(" *\n")
        f.write(f" *  Architecture: {WINDOW_SIZE} -> {H1_SIZE} -> {H2_SIZE} -> {NUM_CLASSES}\n")
        total = (WINDOW_SIZE * H1_SIZE + H1_SIZE +
                 H1_SIZE * H2_SIZE + H2_SIZE +
                 H2_SIZE * NUM_CLASSES + NUM_CLASSES)
...
def main():
    ...
    parser.add_argument("--data", type=str, default=None,
                        help="Path to real FSR CSV data (optional)")
