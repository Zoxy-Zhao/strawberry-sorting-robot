...
{
    .angle_deg = {135U, 70U, 70U, 150U, 80U, 110U}
};

static const robot_pose_t g_pose_pre_grasp =
{
    .angle_deg = {126U, 117U, 86U, 135U, 80U, 110U}
};
...
{
    .angle_deg = {176U, 132U, 83U, 135U, 80U, 110U}
};

static const robot_pose_t g_pose_place_b =
{
    .angle_deg = {201U, 132U, 83U, 135U, 80U, 110U}
};

static const robot_pose_t g_pose_place_c =
{
    .angle_deg = {223U, 132U, 83U, 135U, 80U, 110U}
};
...
static char         g_target_classification = '\0';
static robot_pose_t g_current_pose          = { .angle_deg = {135U, 70U, 70U, 150U, 80U, 110U} };
static robot_pose_t g_teach_pose            = { .angle_deg = {135U, 70U, 70U, 150U, 80U, 110U} };
...
static char const * classification_name(char cls)
{
    ...
    {
        case 'A': return "ripe";
        case 'B': return "unripe";
        case 'C': return "damaged";
        default:  return "unknown";
    }
}
...
static fsp_err_t robot_apply_pose_internal(robot_pose_t const * p_pose, bool allow_abort)
{
    ...
    for (i = 0U; i < SERVO_COUNT; i++)
    {
        ...
        g_current_pose.angle_deg[i] = p_pose->angle_deg[i];

        if (!delay_with_service(SERVO_MOVE_GAP_MS, allow_abort))
        {
            return FSP_ERR_ABORTED;
        }
    }
...
void hal_entry(void)
{
    ...
    while (1)
    {
        ...
        switch (g_state)
        {
            ...
            case STATE_GRASP:
            {
                if (apply_pose_and_wait("GRASP", &g_pose_grasp, GRASP_WAIT_MS))
                {
            ...
            case STATE_PLACE:
            {
                ...
                if (apply_pose_and_wait(p_pose_name, p_place_pose, PLACE_WAIT_MS))
                {
                    state_set(STATE_RETURN);
                }
                break;
