...
#define WRIST_HORIZONTAL_DEG   165U   /* 横向抓取时腕部旋转角度 (默认80° + 85°) */
...
static volatile bool g_target_horizontal     = false;   /* true = 草莓横向，需要旋转腕部 */
    ...
    switch (cls)
    {
        ...
        case 'C':
        {
            ...
            if (STATE_BELT_RUN == g_state)
            {
                ...
                g_target_horizontal = false;  /* 默认竖向，等后续 H 字符覆盖 */
        ...
        case 'H':
        {
            /* 横向标记：紧跟在 A/B/C 之后，如 "AH" */
            if (('\0' != g_target_classification) &&
                (STATE_BELT_STOP == g_state || STATE_PRE_GRASP == g_state))
            {
                g_target_horizontal = true;
                printf("[PI] Horizontal orientation detected\r\n");
            }
            else
            {
                printf("[PI] 'H' ignored in state %s\r\n", state_name(g_state));
            }
            break;
        }
    ...
    while (1)
    {
        ...
        switch (g_state)
        {
            ...
            case STATE_GRASP:
            {
                ...
                /*         If horizontal, rotate wrist 90° */
                {
                    robot_pose_t grasp_open = g_pose_grasp;
                    grasp_open.angle_deg[JOINT_GRIPPER] = g_current_pose.angle_deg[JOINT_GRIPPER];
                    if (g_target_horizontal)
                    {
                        grasp_open.angle_deg[JOINT_WRIST_ROTATE] = WRIST_HORIZONTAL_DEG;
                        printf("[GRASP] Horizontal: wrist rotate → 170°\r\n");
                    }
            ...
            case STATE_LIFT:
            {
                /* If wrist was rotated for horizontal grasp, rotate back */
                robot_pose_t lift_pose = g_pose_lift;
                if (g_target_horizontal)
                {
                    lift_pose.angle_deg[JOINT_WRIST_ROTATE] = WRIST_HORIZONTAL_DEG;
                }
                if (apply_pose_and_wait("LIFT", &lift_pose, LIFT_WAIT_MS))
                {
                    /* 抬起后恢复腕部到默认角度（用插值，防止跳变甩出草莓） */
                    if (g_target_horizontal)
                    {
                        robot_pose_t restore = g_current_pose;
                        restore.angle_deg[JOINT_WRIST_ROTATE] = g_pose_lift.angle_deg[JOINT_WRIST_ROTATE];
                        robot_apply_pose(&restore);
                        printf("[LIFT] Wrist rotate restored → %u°\r\n",
                               (unsigned) g_pose_lift.angle_deg[JOINT_WRIST_ROTATE]);
                    }
