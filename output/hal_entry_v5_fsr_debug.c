...
static bool fsr_grasp_with_feedback(void)
{
    ...
    printf("[FSR] Baseline R=%u | Start %u° → %u° min\r\n",
           (unsigned) baseline_r,
           (unsigned) angle, (unsigned) GRIPPER_CLOSE_MIN_DEG);
    printf("[FSR] R_Contact=%u  R_Target=%u  MaxAfter=%u\r\n",
           (unsigned) FSR_R_CONTACT_DELTA, (unsigned) FSR_R_TARGET_DELTA,
           (unsigned) FSR_MAX_AFTER_CONTACT);
    ...
    while (angle > GRIPPER_CLOSE_MIN_DEG)
    {
        ...
        if (angle % 5U == 0U)
        {
            printf("[FSR] %u° L=%u R=%u(d%u) af=%u%s\r\n",
                   (unsigned) angle,
                   (unsigned) cur_l,
                   (unsigned) cur_r, (unsigned) delta_r,
        ...
        {
            contacted = true;
            printf("[FSR] Contact at %u° (R_delta=%u), closing %u° more\r\n",
                   (unsigned) angle, (unsigned) delta_r,
                   (unsigned) FSR_MAX_AFTER_CONTACT);
        }
        ...
        if (contacted)
        {
            ...
            {
                printf("[FSR] Target reached at %u°! R_delta=%u (after %u°)\r\n",
                       (unsigned) angle, (unsigned) delta_r, (unsigned) after_count);
                return true;
            }
            ...
            {
                printf("[FSR] Max reached, locked at %u° R_delta=%u\r\n",
                       (unsigned) angle, (unsigned) delta_r);
                return true;
            }
        }
    ...
    printf("[FSR] No contact, reached min %u°\r\n",
           (unsigned) GRIPPER_CLOSE_MIN_DEG);
    return false;
}
