    ...
    try:
        while True:
            ...
            if pi_state == PI_STATE_SEND_G:
                ...
                # 判断朝向：高/宽 > 阈值为横向（摄像头俯视，高>宽说明草莓横跨传送带）
                bbox_w = best_det.x2 - best_det.x1
                bbox_h = best_det.y2 - best_det.y1
                aspect = bbox_h / bbox_w if bbox_w > 0 else 1.0
                full_cmd = best_det.mcu_cmd + ("H" if aspect > config.HORIZONTAL_ASPECT_RATIO else "")

                # 连续帧确认：同一类别+同一朝向连续 N 帧才触发
                if full_cmd == confirm_cmd:
                    confirm_count += 1
                else:
                    confirm_count = 1
                    confirm_cmd = full_cmd

                if confirm_count < config.DETECT_CONFIRM_FRAMES:
                    logger.info("确认中: %d/%d", confirm_count, config.DETECT_CONFIRM_FRAMES)
                    continue

                # 确认通过，发送分类指令（朝向已在确认逻辑中确定）
                confirm_count = 0
                cmd = confirm_cmd
                confirm_cmd = None
                # 分字节发送，MCU 单字节缓冲区需要间隔消化
                comm.ser.write(cmd[0].encode("ascii"))
                comm.ser.flush()
                if len(cmd) > 1:
                    time.sleep(0.02)  # 20ms，等 MCU 主循环取走第一个字节
                    comm.ser.write(cmd[1:].encode("ascii"))
                    comm.ser.flush()
                logger.info("TX → MCU: %s (已确认，%s)", cmd,
                            "横向" if cmd.endswith("H") else "竖向")
